...
class Settings:
    ...
    dz_limit_m: float = 50e-6
    barrier_limit_ev: float = 1e-3
    lateral_limit_m: float = 10e-6
...
class Individual:
    ...
    def objectives(self) -> np.ndarray:
        """One objective only: minimise RF pseudopotential barrier in meV."""
        if not self.valid:
            return np.array([np.inf], dtype=float)

        return np.array(
            [
                self.result["barrier_ev"] * 1e3,
            ],
            dtype=float,
        )
...
def style_score(
    ...
    """Barrier-only parent preference.

    style is retained only because tournament still passes it.
    It does not affect the physical optimisation objective.
    """
    del style
    ...
    return float(
        individual.result["barrier_ev"] * 1e3
    )
